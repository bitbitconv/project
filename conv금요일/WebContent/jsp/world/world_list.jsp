...
$(document).ready(function(){
    $("button").click(function(){
        $.ajax({url:"/conv/com/conv/recipe/controller/recipeaddcontroller", success: function(result){
            $("#list").html(result);
        }});
    });
});
...
<div class="w3-container">
  ...
  <div class="w3-center">
    <h2>세계의 편의점</h2>
    <p w3-class="w3-large">세계의 특이한 편의점 구경하기 !</p>
  </div>
  <a href = "/conv/jsp/world/world_insert.jsp">
<button class="w3-btn w3-xlarge w3-blue w3-hover-white" style="font-weight:900;">글쓰기</button></a>
</div>

<div class="grid are-images-unloaded">
<form action ="/conv/conv/post">   
    <c:forEach var ="post" items="${list}" >
    <div class="w3-row-padding w3-third w3-margin-top">
     <div class="col-xs-6 col-md-4" width ="33%" float = "left" >
     
        글번호 :<button class="w3-bar-item w3-button" name="no" 
        value="<c:out value="${post.no}" />">
                   <c:out value="${post.no}" />.
            </button></td>
        <c:out value="${post.id}"/><br> 
       <img  src ="${pageContext.request.contextPath}/${post.photo}"><br>
